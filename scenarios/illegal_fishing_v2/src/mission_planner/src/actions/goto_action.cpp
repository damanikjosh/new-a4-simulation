...
namespace mission_planner
{
    ...
    bool GotoActionHandler::initialize_task(std::shared_ptr<const typename GotoAction::Goal> goal)
    {
        ...
        dummy_item.relative_altitude_m = goal->absolute_altitude_m - home_altitude;
        ...
        mission_item.relative_altitude_m = goal->absolute_altitude_m - home_altitude;
        ...
        if (!ready)
        {
            RCLCPP_ERROR(vehicle_->get_logger(), "Vehicle not ready for Goto mission");
            return false;
        }
    ...
    void GotoActionHandler::stop_task()
    {
    ...
    std::shared_ptr<GotoActionHandler::GotoAction::Result> GotoActionHandler::get_cancel_result()
    {
        std::shared_ptr<GotoAction::Result> result = std::make_shared<GotoAction::Result>();
        result->success = false;
        result->final_distance_to_target_m = feedback_->distance_to_target_m;
        return result;
    }

    std::shared_ptr<GotoActionHandler::GotoAction::Result> GotoActionHandler::get_finish_result()
    {
        std::shared_ptr<GotoAction::Result> result = std::make_shared<GotoAction::Result>();
        result->success = true;
        result->final_distance_to_target_m = feedback_->distance_to_target_m;
    ...
    std::shared_ptr<GotoActionHandler::GotoAction::Feedback> GotoActionHandler::get_feedback()
    {
        if (!current_goal_)
        {
            return feedback_;
        ...
        auto goal = current_goal_->get_goal();
        feedback_->distance_to_target_m = static_cast<float>(
            mission_planner::utils::haversine_distance(
                ...
                goal->latitude_deg, goal->longitude_deg));

        return feedback_;
    }

    bool GotoActionHandler::is_finished()
    {
        return false;
    }
