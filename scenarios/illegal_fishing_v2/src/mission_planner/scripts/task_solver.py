...
def solve_task(task_points, task_reqs, task_done, vehicle_points, logger=print):
    ...
    # Map solution indices back to original task indices
    solution_idx = [np.array(task_avail)[solution_k] for solution_k in solution]
    # Get coordinates using the correct original indices
    solution_points = [task_points[idx] for idx in solution_idx]
    logger('Solution:')
    for k, solution_k in enumerate(solution_idx):
        logger(f'  Vehicle {k}: {solution_k.tolist()}')
